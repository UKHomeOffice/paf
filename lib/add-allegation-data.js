...
const addAdditionalPeople = (eform, additonalpeople) => {
  if (additonalpeople.length > 0) {
    additonalpeople[0].map(item => {
      eform.EformFields.push({FieldName: item.FieldName, FieldValue: item.FieldValue});
    });
    if (additonalpeople.length > 1) {
      eform.AdditionalPeople = additonalpeople.splice(1);
    }
  }
};
...
const addFieldToEform = (eform, fieldName, fieldValue) => {
  if (fieldName === 'images') {
    eform.Attachements = fieldValue;
  } else if (fieldName === 'tbwhoaddperson') {
    addAdditionalPeople(eform, fieldValue);
  } else {
